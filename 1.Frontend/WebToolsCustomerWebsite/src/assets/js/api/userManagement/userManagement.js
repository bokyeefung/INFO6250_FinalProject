...
export default {
    ...
    methods: {
        getAllList(data, successFunc, failedFunc) {
            let that = this;
            this.$axios.get('/api/customer/admin/user').then(result => {
        ...
        addUser(data, successFunc, failedFunc) {
            let that = this;
            this.$axios.post('/api/customer/admin/user', data).then(result => {
                successFunc(result.data);
            }).catch(err => {
                if ('function' === typeof failedFunc) {
                    failedFunc(err.response.data.message);
        ...
        deleteUser(uuid, successFunc, failedFunc) {
            let that = this;
            this.$axios.delete('/api/customer/admin/user/' + uuid).then(result => {
                successFunc(result.data);
            }).catch(err => {
                if ('function' === typeof failedFunc) {
                    failedFunc(err.response.data.message);
