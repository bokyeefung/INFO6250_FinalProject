...
export default {
    ...
    methods: {
        login(data, success, failed) {
            let that = this;
            this.$axios.post('/api/distributor/security/login', data).then(result => {
                that.isLogin = true;
        ...
        logout(success, failed) {
            let that = this;
            this.$axios.post('/api/distributor/security/logout', {}).then(result => {
                that.clearUserInfo();
      ...
      checkLogin(uuid, failed) {
        let that = this;
        this.$axios.post('/api/distributor/security/check/' + uuid).then(() => {
        }).catch(err => {
          if ('function' === typeof failed) {
            failed(err.response.data.message);
          }
